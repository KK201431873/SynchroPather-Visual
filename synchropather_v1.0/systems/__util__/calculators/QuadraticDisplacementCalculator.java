 ...
 */
public class QuadraticDisplacementCalculator extends DisplacementCalculator {
	 ...
	 * @param MA
	 */
	public QuadraticDisplacementCalculator(double targetDisplacement, TimeSpan timeSpan, double MV, double MA) {
	...
	public static double a1int(double t, double K) {
		return ((2-4*K*((K-2)*K+2))*atanh(((K-1)*(K-2*t))/(-2*K*t+(K-1)*K+2*t*t+1))+2*(K-1)*(2*((K-2)*K+2)*t-Math.atan(-2*K+2*t+1)+Math.atan(1-2*t)))/(16*(K-1)*((K-2)*K+2));
	}
